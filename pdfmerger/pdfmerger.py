...
class Handler(httpserver.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.end_headers()
        print("GET") 
        self.wfile.write("<html>GET")
        return       
        with open('../data/example.pdf','r') as f:
	    for l in list(f):
                self.wfile.write(l)
        return 
	...
	print('Client: %s' % str(self.client_address))
        ...
        print('User-agent: %s' % str(self.headers['user-agent']))
	    ...
	    if field_item.filename:
	        ...
	        print('Uploaded %s as "%s" (%d bytes)' % (field, field_item.filename, file_len))
